    ...
    describe('update', () => {
        it('should throw an error if currentPassword is empty', () => {
            ...
            Update.prototype.password(req, res).catch((error: CustomError) => {
                ...
                expect(error.serializeErrors().message).toEqual('Current Password is a required field');
            });
        });
        ...
        it('should throw an error if newPassword is empty', () => {
            const req: Request = authMockRequest(
                {},
                {
                    currentPassword: 'manny1234567',
                    newPassword: '',
                    confirmPassword: 'manny212345678'
                }
            ) as Request;
            const res: Response = authMockResponse();
            Update.prototype.password(req, res).catch((error: CustomError) => {
                expect(error.statusCode).toEqual(400);
                expect(error.serializeErrors().message).toEqual('New password is a required field');
            });
        });
